...
class MessageCenter(object):
    ...
    def consume_message(self, decrypted_message: node_pb2.RawMessage,
    ...
    def find_dna_to_reach_destination(self, relay_message_request: node_pb2.RelayMessageRequest,
                                      ...
                                      topic_id: str, message_dna_length=128, wait_time_out_ms=2000,
        ...
        relay_requests_with_different_dna = []
        dna_to_try = message_utils.get_message_dna_candidates_for_95_pct_reachability(message_dna_length)

        for relay_request in relay_requests_with_different_dna:
            self.relay_message_to_connections(relay_request, peer_node_hash_to_channels)
    ...
    def process_acknowledgement_message(self, decrypted_message: node_pb2.RawMessage,
                                        original_relay_request: node_pb2.RelayMessageRequest):
    ...
    def __create_handshake_encrypted_message(self, destination_node_public_key_hex_str: str, return_destination_id,
        ...
        return message_utils.encrypt_raw_message(unencrypted_output, destination_node_public_key_hex_str)

    def __check_if_working_dna_is_found(self, topic_id, on_success, on_failure):
        with self.lock:
            if topic_id in self.destination_id_to_working_dna_candidates and len(
                    self.destination_id_to_working_dna_candidates[topic_id]) > 0:
                on_success(self.destination_id_to_working_dna_candidates[topic_id])
            else:
                on_failure()
